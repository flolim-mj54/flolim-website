...
const SupportArchive = () => {
  // 💡 [Map 데이터]
  const brochures = [
    { id: 1, title: '플로림 통합 브로슈어 (2026)', desc: '스마트 시티 및 빌딩 솔루션 통합 안내서 (PDF, 5.2MB)', iconColor: 'text-red-400', icon: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z" /> },
    { id: 2, title: 'ESCO 사업 제안서', desc: '초기 투자비 0원 KEPCO 연계 사업 안내서 (PDF, 3.8MB)', iconColor: 'text-blue-400', icon: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /> },
  ];

  const manuals = [
    { id: 1, title: '단말기 기술 사양서', desc: 'LoRa / NB-IoT Node Spec' },
    { id: 2, title: '플랫폼 사용자 매뉴얼', desc: '관제 대시보드 사용 가이드' },
    { id: 3, title: '공식 인증 및 성적서', desc: 'KILT 등 통합 인증서 모음' },
  ];
        ...
        <section className="bg-slate-900/50 backdrop-blur-md rounded-[2.5rem] p-6 md:p-10 lg:p-16 shadow-2xl border border-slate-800 mb-16 relative overflow-hidden">
              ...
              {brochures.map((b) => (
                <div key={b.id} className="bg-[#050b14] p-6 md:p-8 rounded-3xl border border-slate-700 shadow-inner flex flex-col sm:flex-row items-center sm:items-start gap-4 md:gap-6 group relative overflow-hidden cursor-default">
                  ...
                  <div className="flex-1 text-center sm:text-left relative z-10 w-full">
                    ...
                    <button className="w-full sm:w-auto px-4 py-2 bg-slate-800 hover:bg-flolim active:scale-95 hover:text-slate-900 text-slate-300 text-xs font-bold rounded-lg transition-all border border-slate-600 hover:border-flolim flex items-center justify-center gap-2">
          ...
          <div className="relative z-10">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
              {manuals.map((m) => (
                <div key={m.id} className="bg-[#050b14] p-5 md:p-6 rounded-2xl border border-slate-700 shadow-inner group relative overflow-hidden cursor-default">
                  ...
                  <button className="w-full py-2.5 bg-slate-800 hover:bg-flolim active:scale-95 hover:text-slate-900 text-slate-300 text-xs font-bold rounded-lg transition-all border border-slate-600 hover:border-flolim relative z-10">다운로드</button>
                </div>
              ))}
            </div>
          </div>
