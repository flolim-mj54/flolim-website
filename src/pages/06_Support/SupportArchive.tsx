      ...
      <div className="container mx-auto px-4 max-w-6xl mt-10">
        <section className="bg-slate-900/50 backdrop-blur-md rounded-[2.5rem] p-6 md:p-10 lg:p-16 shadow-2xl border border-slate-800 mb-16 relative overflow-hidden">
          ...
          <div className="mb-12 md:mb-16 relative z-10">
            <h2 className="text-xl md:text-2xl font-bold text-white mb-6 border-l-4 border-flolim pl-3 break-keep">
              회사 소개 및 ESCO 사업 소개
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {brochures.map((b) => (
                ...
                <div
                  key={b.id}
                  className="bg-[#050b14] p-6 md:p-8 rounded-3xl border border-slate-700 shadow-inner flex flex-col h-full group relative overflow-hidden cursor-default"
                >
                  ...
                  <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4 md:gap-5 mb-6 relative z-10 flex-1 w-full">
                    ...
                    <div className="text-center sm:text-left w-full pt-1">
                      <h3 className="text-base md:text-lg font-bold text-white mb-2 group-hover:text-flolim transition-colors break-keep">
                        {b.title}
                      </h3>
                      <p className="text-[11px] md:text-xs text-slate-400 leading-relaxed break-keep">
                        {b.desc}
                      </p>
                    </div>
                  </div>
          ...
          <div className="relative z-10">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
              {manuals.map((m) => (
                <div
                  key={m.id}
                  className={`bg-[#050b14] p-5 md:p-6 rounded-2xl border ${m.isReady ? "border-slate-700 hover:border-flolim/50" : "border-slate-800/50"} shadow-inner group relative overflow-hidden cursor-default flex flex-col justify-between h-full transition-colors`}
                >
                  {m.isReady && (
                  ...
                  <div className="relative z-10 flex justify-between items-start mb-6">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <h3
                          className={`text-sm md:text-base font-bold ${m.isReady ? "text-white group-hover:text-flolim" : "text-slate-500"} transition-colors break-keep`}
                        >
                          {m.title}
                        </h3>
                        {!m.isReady && (
                      ...
                      <p
                        className={`text-[10px] md:text-xs break-keep ${m.isReady ? "text-slate-400" : "text-slate-600"}`}
                      >
                        {m.desc}
                      </p>
                    </div>
                    ...
                    <div
                      className={`w-10 h-10 md:w-12 md:h-12 bg-white rounded-xl flex items-center justify-center border shrink-0 p-1.5 md:p-2 transition-all ${m.isReady ? "border-slate-200 shadow-sm" : "border-slate-800 opacity-30 grayscale"}`}
                    >
                      <img
                        src={m.img}
                        alt="PDF 문서"
