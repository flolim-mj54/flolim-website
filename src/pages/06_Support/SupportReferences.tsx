...
const SupportReferences = () => {
  ...
  const buildingCases = [
    { id: 4, tag: '공장 및 상업시설', title: '한국전자통신연구원 실증', desc: '스마트조명 플랫폼 개별/그룹 제어 적용', prefix: '최대', percent: '74.1', unit: '%', label: '절감' },
            ...
            {cityCases.map((c) => (
              <div key={c.id} className="bg-[#050b14] p-6 md:p-8 rounded-3xl border border-slate-700 shadow-inner hover:border-flolim/50 active:scale-[0.98] transition-all duration-300 group relative overflow-hidden flex flex-col h-full cursor-default">
                ...
                <div className="relative z-10 flex flex-col h-full">
                  <span className="inline-block px-3 py-1 bg-slate-800 text-slate-400 text-[10px] md:text-xs font-bold rounded-lg mb-4 border border-slate-700 self-start">{c.tag}</span>
                  ...
                  <p className="text-xs md:text-sm text-slate-500 font-medium mb-6 break-keep flex-grow">{c.desc}</p>
                  
                  {/* 💡 [수정] 모바일/태블릿에서 텍스트가 예쁘게 줄바꿈 되도록 flex-wrap 적용 */}
                  <div className="flex flex-wrap items-end gap-x-2 gap-y-1 mt-auto">
                    ...
                    <span className="text-3xl md:text-4xl font-black text-flolim tracking-tight drop-shadow-[0_0_15px_rgba(24,169,198,0.3)] whitespace-nowrap">
                      {c.percent}<span className="text-lg md:text-xl font-medium ml-0.5">{c.unit}</span>
                    </span>
