    ...
    },
    {
      ...
      title: (
        <span className="break-words">
          Governance &<br className="hidden lg:block" /> Economy
        </span>
      ),
  ...
  const strengths = [
    {
      id: 1,
      title: "위변조 불가능한 신뢰성",
      desc: "하드웨어 스마트 미터기에서 측정한 원본 데이터를 사용하여, 데이터 무결성이 100% 보장됩니다.",
      icon: (
        <path
          strokeLinecap="round"
          strokeLinejoin="round"
    ...
    },
    {
      id: 2,
      title: "스케줄 기반 자동 발행",
      desc: "원하는 주기(월간, 분기, 연간)를 설정하면, 플랫폼이 자동으로 성과를 분석하여 PDF 및 Excel 리포트를 완성합니다.",
      icon: (
        <path
          strokeLinecap="round"
          strokeLinejoin="round"
    ...
    },
    {
      id: 3,
      title: "글로벌 표준 (RE100)",
      desc: "글로벌 RE100, K-ETS 지침에 맞춘 Scope 2 배출량 산정 공식을 정확하게 시스템에 반영합니다.",
      icon: (
        <path
          strokeLinecap="round"
          strokeLinejoin="round"
        ...
        <section className="bg-slate-900/50 backdrop-blur-md rounded-[2.5rem] p-6 md:p-10 lg:p-16 shadow-2xl border border-slate-800 mb-16 relative overflow-hidden min-h-[500px] md:min-h-[700px] flex flex-col">
          <div className="text-center mb-10 md:mb-16 relative z-20 px-2">
            ...
            <p className="text-slate-400 text-xs md:text-sm break-keep">
              데이터가 증명하는 객관적인 탄소 감축 및 예산 절감 성과를
              확인하세요.
            </p>
          </div>
          ...
          <div className="perspective-[1200px] flex-1 flex items-center justify-center relative z-10 w-full pb-6 md:pb-10">
            <div
              ...
              style={{
                ...
                transform: isUnpacked
                  ? "rotateX(0deg) translateY(0) scale(1)"
                  : "rotateX(30deg) translateY(60px) scale(0.85)",
              }}
            >
              <div
                className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[110%] h-[110%] bg-flolim/20 blur-[100px] rounded-full transition-all duration-[2000ms] z-0 pointer-events-none ${isUnpacked ? "scale-100 opacity-100" : "scale-50 opacity-0"}`}
              ></div>
        ...
        <section className="mb-16 md:mb-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 relative z-10">
            {esgIndicators.map((ind) => (
              <div
                key={ind.id}
                className={`bg-[#050b14] p-6 md:p-8 rounded-3xl border border-slate-700 shadow-inner transition-colors group relative overflow-hidden cursor-default active:scale-[0.98] flex flex-col ${ind.classes.border}`}
              >
                <div
                  className={`absolute -right-6 -top-6 w-24 h-24 rounded-full z-0 group-hover:scale-150 transition-transform duration-700 pointer-events-none ${ind.classes.bgGlow}`}
                ></div>
                <div
                  className={`w-10 h-10 md:w-12 md:h-12 bg-slate-800 rounded-xl flex items-center justify-center mb-4 md:mb-5 transition-colors relative z-10 border border-slate-600 ${ind.classes.iconBox}`}
                >
                  <svg
                ...
                <h3
                  className={`text-base md:text-lg font-bold text-white mb-1 md:mb-2 transition-colors relative z-10 break-keep ${ind.classes.titleHover}`}
                >
                  {ind.title}
                </h3>
                <p
                  className={`font-bold text-xs md:text-sm mb-2 relative z-10 break-keep ${ind.classes.subtitle}`}
                >
                  {ind.subtitle}
                </p>
                <p className="text-slate-400 font-light text-xs md:text-sm leading-relaxed relative z-10 break-keep flex-grow">
                  {ind.desc}
                </p>
              </div>
            ))}
        ...
        <section className="mb-20">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 relative z-10">
            {strengths.map((str) => (
              <div
                key={str.id}
                className="bg-[#050b14] p-6 md:p-8 rounded-3xl border border-slate-700 shadow-inner hover:border-flolim/50 active:scale-[0.98] active:border-flolim transition-colors group relative overflow-hidden cursor-default flex flex-col"
              >
                ...
                <h3 className="text-base md:text-lg font-bold text-white mb-2 group-hover:text-flolim transition-colors relative z-10 break-keep">
                  {str.title}
                </h3>
                <p className="text-slate-400 font-light text-xs md:text-sm leading-relaxed relative z-10 break-keep flex-grow">
                  {str.desc}
