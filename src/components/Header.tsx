...
const Header = () => {
  ...
  const menuItems = [
    ...
    },
    {
      ...
      subItems: [
        ...
        { name: "스마트 LED 제품소개", path: "/smart-building/led" },
  ...
  return (
    <>
      ...
      >
        <div className="container mx-auto px-4 max-w-7xl">
          <div className="flex items-center justify-between">
            <Link
              to="/"
              className="flex items-center group z-50 active:scale-95 transition-transform"
            >
              <span className="text-xl md:text-2xl font-black tracking-tight">
                <span className="text-flolim drop-shadow-[0_0_10px_rgba(24,169,198,0.5)] group-hover:text-cyan-400 transition-colors">
                  FLO
                </span>
      ...
      <div
        className={`fixed inset-0 bg-[#020617]/95 backdrop-blur-lg z-40 lg:hidden transition-opacity duration-300 ${mobileMenuOpen ? "opacity-100 visible" : "opacity-0 invisible"}`}
      >
        <div className="h-full overflow-y-auto pt-20 pb-10 px-6">
          <div className="flex flex-col gap-5">
            {menuItems.map((item, idx) => (
              <div key={idx} className="border-b border-slate-800 pb-3">
                <h3 className="text-flolim font-bold text-base mb-3 break-keep">
                  {item.title}
                </h3>
