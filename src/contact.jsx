...
const Contact = () => {
  ...
  const onSubmit = async (event) => {
    ...
    // 폼에 입력된 데이터(이름, 이메일, 내용 등)를 싹 긁어모읍니다.
    const formData = new FormData(event.target);

    // 🚀 핵심: 방금 새로 발급받은 moojeong54@flolim.com 전용 Access Key를 넣습니다!
    ...
    try {
      ...
      if (data.success) {
        ...
        event.target.reset(); // 성공하면 입력창을 깨끗하게 비워줍니다.
      } else {
        ...
        <aside className="w-full md:w-64 flex-shrink-0">
          <div className="border border-slate-300">
            ...
            <ul className="flex flex-col">
              <li className="border-b border-slate-200"><a href="#" className="flex items-center justify-between px-5 py-4 bg-cyan-50 text-[#1eb4c8] font-bold">온라인 문의 <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="square" strokeLinejoin="miter" strokeWidth="2" d="M9 5l7 7-7 7"></path></svg></a></li>
              <li className="border-b border-slate-200 last:border-0"><a href="#" className="flex items-center justify-between px-5 py-4 text-slate-600 hover:text-[#1eb4c8] hover:bg-slate-50 transition-all">자주 묻는 질문</a></li>
            </ul>
          </div>
        </aside>
        ...
        <section className="flex-1 min-w-0">
          ...
          <form onSubmit={onSubmit} className="bg-slate-50 border border-slate-200 p-8">
            <div className="space-y-6">
              ...
              {result && (
                <div className={`p-4 font-bold ${result.includes("성공") ? "text-green-600 bg-green-50" : "text-[#1eb4c8] bg-cyan-50"}`}>
                  {result}
                </div>
              )}
